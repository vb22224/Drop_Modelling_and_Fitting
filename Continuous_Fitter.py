...
def trace_plotter(time, total_trace, av_times, averaged_data, selfcharge_trace=[], precharge_trace=[], adjust_by_mass=False, dpi=600):
    ...
    if selfcharge_trace is not None and len(selfcharge_trace) > 0:
        ax.plot(time, selfcharge_trace, '-', label='self-charging', linewidth=1.0) 

    if precharge_trace is not None and len(precharge_trace) > 0:
        ax.plot(time, precharge_trace, '-', label='pre-charging', linewidth=1.0) 
        ax.plot([time[0], time[-1]], [0, 0], '--', linewidth=1.0, color='black')    

    ax.plot(time, total_trace, '-', label='predicted trace', linewidth=1.0) 
    ax.plot(av_times, averaged_data, '-', label='measured trace', linewidth=1.0)

    ax.set_xlim([time[0], time[-1]])
    ax.set_xlabel('Time / s')
    
    if adjust_by_mass:
        ax.set_ylabel('Specific Charge / pC g$^{-1}$')
    else:
...
if __name__ == "__main__":
    ...
    remove_files = ['63-75_1.txt', '63-75_4.txt', '63-125_5.txt', '63-125_4.txt', 'StHelens_1.txt', 'Atitlan_3.txt', 'StHelens_3.txt']
    lol_labels = ["Min", "Max"]
    spec_cat = ['Atitlan'] # specify categories to include if input type is categorised (file names before the underscore), leave empty to include all
    ...
    adjust_for_decay, time_const = False, 310.5 # in [s] and comes from the conductivity of air (David's = 310.5)
    ...
    initial_guess = [-0.01567612366319226, 7.092560012873394, -0.2] # Charge multiplier, precharge ratio, and time shift initial guesses
    ...
    mode_sizes = [0.011893307824956314, 0.8835042718769689, 0.10044386279111804]
    mode_means = [29.996231224439516, 89.2600723016883, 180.02112020622215]
    mode_stds = [0.05879818332068189, 0.12622046257599862, 0.125086014126218]
    truncate = float('inf')
    
    # Charge Fit
    fit_type = 'complex' # Simple or complex depending on the shape of the output. Complex only needs a and b
    a, b, c = 0.0017289575439138408, -387.8118218628825, 0
    
    # Time Fit
    p_p = 2425.055 # Density of particle in kg m^-3 (around 1500 for ash, 2670 for Laradorite and 8940 for Cu)
    trace_time = 20 # The time the trace is recorded for, allows the small particles where there is numerical instability to be cut off
    convert_time = ['N', 0.87, 8.797, 0.541] # [convert?, constant, alpha, beta] First option none (N), constant (C) or exponential (E) for whether to convert the distribution and the fitting paramters if so
    ...
    # save_name = 'Atitlan_fit'
    # df = pd.DataFrame({'Time': time, 'Total_Trace': total_trace})
    # df.to_csv(path + "\\" + save_name + '.csv', index=False)
