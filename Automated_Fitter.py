...
if __name__ == "__main__":
    ...
    lol_labels = ["Min", "Max"]
    spec_cat = ['StHelens'] # specify categories to include if input type is categorised (file names before the underscore), leave empty to include all
    ...
    initial_guess = [-0.025, 5, -0.05] # Charge multiplier, precharge ratio, and time shift initial guesses
    ...
    a, b, c = 0.010213607341602917, -728.2377396100616, 0
    
    # Time Fit
    p_p = 2721.158 # Density of particle in kg m^-3 (around 1500 for ash, 2670 for Laradorite and 8940 for Cu)
    trace_time = 10 # The time the trace is recorded for, allows the small particles where there is numerical instability to be cut off
